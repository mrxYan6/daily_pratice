...
#include <bits/stdc++.h>
    ...
    std::cout.tie(nullptr);
        ...
        Poly F(f), G(g);
        Poly Q = (F * G.inv(n - m + 1)).modxk(n - m + 1);
        std::reverse(Q.a.begin(), Q.a.end());
        std::reverse(F.a.begin(), F.a.end());
        std::reverse(G.a.begin(), G.a.end());
        Poly R = F - Q * G;
        for (int i = 0; i <= n - m; ++i)std::cout << Q[i] << ' ';
        std::cout << '\n';
        for (int i = 0; i <= m - 1; ++i)std::cout << R[i] << ' ';
        return 0;
    }
    freopen("in.txt", "r", stdin);
#endif
