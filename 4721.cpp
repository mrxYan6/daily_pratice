...
int main() {
    ...
    int n;
    std::cin >> n;
    std::vector<Z> a(n), h(n, 0);
    h[0] = 1;
    for (int i = 1; i < n; ++i)std::cin >> a[i];
    Poly g(a), f(h);
    std::function<void(int, int)> solve = [&](int l, int r) -> void {
        if (l == r) {
            return;
        }
        int mid = (l + r) >> 1;
        solve(l, mid);
//        f.divxk(l).modxk(mid - l + 1)
//        g.modxk(r - l + 1)
        Poly a = Poly(std::vector<Z>(f.a.begin() + l, f.a.begin() + mid + 1)) * Poly(std::vector<Z>(g.a.begin(), g.a.begin() + r - l + 1));//计算左半边到右半边的贡献
