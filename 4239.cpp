...
long long getint(Z &z) {
    long long x = (z.B.val() - z.A.val() + mod2) % mod2 * power(mod1, mod2 - 2, mod2) % mod2 * mod1 + z.A.val();
    ll ans = ((z.C.val() - x % mod3 + mod3) % mod3 * power(mod1 * mod2 % mod3, mod3 - 2, mod3) % mod3 * (mod1 * mod2 % P) % P + x) % P;
    return ans;
}
...
struct Poly {
    ...
    Poly inv(int m) const {
        Poly x{power(a[0].A.val(), P - 2, P)};
        int k = 1;
        while (k < m) {
            k *= 2;
            Poly tmp = modxk(k) * x;
            for (int i = 0; i < k; ++i) {
                tmp[i] = Z(P - getint(tmp[i]) % P);
            }
            tmp[0] = Z(1);
            x = (x * tmp).modxk(k);
            for (int i = 0; i < k; ++i) {
...
int main() {
    ...
    Poly ans = Poly(f).inv(n);
    for (int i = 0; i < n; ++i) {
        std::cout << getint(ans[i]) << ' ';
    }
    return 0;
}