...
struct Poly {
    ...
    Poly pow(int k, int k2, int m) const {
        int i = 0;
        ...
        return (f.log(m - i * k) * k).exp(m - i * k).mulxk(i * k) * power(v, k2);
    }
...
int main() {
    ...
    if (k.val() == 0) {
        if (k2.val() == 0) {
            ...
            std::cout << power(Z(a[0]), k2.val());
            for (int i = 1; i < n; ++i)std::cout << " 0";
        }
    } else {
        Poly ans(a);
        ans = ans.pow(k.val(), k2.val(), n);
