...
//    std::cerr << tot << '\n';
    Z tot_not_legal = 0;
    for (int j = 0; j < m; ++j) {
        std::vector<std::vector<Z>> not_legal(n + 1, std::vector<Z>(2 * n + 1, 0));
        not_legal[0][n] = 1;
        for (int i = 1; i <= n; ++i) {
            for (int k = -n; k <= n; ++k) {
                not_legal[i][k + n] += not_legal[i - 1][k + n];
                if (k < n) not_legal[i][k + n] += (sum[i] - a[i][j]) * not_legal[i - 1][k + n + 1];
                if (k > -n)not_legal[i][k + n] += a[i][j] * not_legal[i - 1][k + n - 1];
            }
        }
//        for (int i = 0; i <= n; ++i) {
//            for (int k = -n; k <= n; ++k)std::cerr << not_legal[i][k + n] << ' ';
//            std::cerr << '\n';
//        }
        for (int i = n + 1; i <= 2 * n; ++i)tot_not_legal += not_legal[n][i];
    }
    std::cout << tot - tot_not_legal << '\n';
    return 0;
}