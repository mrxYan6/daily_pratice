...
std::vector<Pd> hp(std::vector<Ld>& lines) {
...
void solve() {
    ...
    for (int i = 0; i < n; ++i) {
        int m;
        std::cin >> m;
        std::vector<Pd> ps(m + 1);
        for (int i = 0; i < m; ++i) std::cin >> ps[i];
        ps[m] = ps[0];
        for (int i = 1; i <= m; ++i) {
            lines.emplace_back(ps[i - 1], ps[i]);
        }
    }
    ...
    db area = 0;
    for (int i = 1; i < m; ++i) {
        area += cross(ans[i - 1], ans[i]);
    }
    std::cout << std::fixed << std::setprecision(3) << std::abs(area) / 2 << '\n';
}
